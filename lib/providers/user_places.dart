...
class UserPlacesNotifier extends StateNotifier<List<Places>>{
  ...
  Future<void> loadPlaces() async{
    ...
    final placeList = data.map((row){
      return Places(id: row['id'] as String,title: row['title'] as String,image: File(row['image'] as String),
  ...
  void addPlace(String title,File image,PlaceLocation location) async{
    ...
    final place = Places(title: title,image: copiedImg,location: location);
    final db = await _getDataBase();
    db.insert('user_places',{
      'id':place.id,
      'title':place.title,
      'image':place.image.path,
      'lag':place.location.latitude,
      'lng':place.location.longitute,
